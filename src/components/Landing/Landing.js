...
function Landing() {
    const { theme, drawerOpen, isIntroFinished } = useContext(ThemeContext);
    const [displayText, setDisplayText] = useState('');
    const [isTyping, setIsTyping] = useState(false); // Start false

    useEffect(() => {
        if (!headerData.description || !isIntroFinished) return; // Wait for intro
    ...
    const useStyles = makeStyles((t) => ({
        ...
        contactBtn: {
            ...
            '&:hover': {
                ...
                color: theme.tertiary,
                border: `2px solid ${theme.tertiary}`,
                boxShadow: `0 20px 40px -15px ${theme.tertiary}80, 0 0 20px ${theme.tertiary}50`,
            },
            '&:hover::before': {
                opacity: 1,
            },
